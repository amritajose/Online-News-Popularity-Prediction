...
# The dataset for the project is about the Online News Popularity of Mashable articles; the source of the dataset is UCI Machine Learning Repository. 
...
#  want to predict the popularity of a given article based on the given attributes. 
...
# The model can predict the popularity of an online news article and determine whether it will draw significant attention. 
# Finding out the relationship beten the response variable (Number of shares) and other predictors, a model can be built that helps us decide the content of articles or the timing of the release to maximize the popularity of the articles. 
...
# Both Accuracy and F1 score to determine the best model. 
...
#Hover, all variables have low correlation with the target variable - number of shares.
...
# 3. Based on the distributions, it can be seen that the data is sked to some extent.

# Determining the effect of ekdays and ekends on number of shares
for (i in 31:37){
  boxplot(log(news$shares) ~ (news[,i]), xlab=names(news)[i] , ylab="shares")
}
# Observing the above box plots, it can be said that ekdays show some effect on shares.Hover, they can also be chosen to be not considered due to the very small effect. 
# Hover, ekends seem to have more considerable effect.
...
# KNN Optimization, checking the best value of k beten 1:30
i=1
k.optm=1
for (i in 1:30){
...
# From the output  can see that for K = 25,  achieve the maximum accuracy, i.e. 58.88%.

# Accuracy plot
plot(k.optm, type="b", xlab="K- Value",ylab="Accuracy level")
# The above graph shows that for ‘K’ value of 25  get the maximum accuracy.
...
# In this case, AUC is 0.590, it means the model has no discrimination capacity to distinguish beten the two classes.
...
# With Classification Trees,  report the average outcome at each leaf of our tree. Hover, Instead of just taking the majority outcome to be the prediction,  can compute the percentage of data in a subset of each type of outcome.
...
# A strategy to limit this overfitting is to prune back the tree resulting to a simpler tree with fer splits and better interpretation at the cost of a little bias

#  can use the following arguments in the function train() [from caret package]:
# trControl, to set up 10-fold cross validation
# tuneLength, to specify the number of possible cp values to evaluate. Default value is 3, here ’ll use 10.
...
# with an overall accuracy of 64.15%, which is comparable to the accuracy (62.86%) that  have obtained with the full tree. 
...
# AUC shows the predictive ability of the model, the higher the AUC the more predictive the marker.
# In this case, AUC is 0.671, it means there is 67.1% chance that model will be able to distinguish beten the two classes.
...
confnb
#  re able to classify 3740 out of 4062 "not popular" cases correctly
#  re able to classify 3529 out of 3870 "popular" cases correctly
...
confnb2
#  re able to classify 3912 out of 4062 "not popular" cases correctly
#  re able to classify 3859 out of 3870 "popular" cases correctly
...
#While performing the SVM model,  re running out of memory and R kept crashing hence  could not publish the result for the model or do any further analysis.
...
# The aim is to establish a linear relationship beten the predictor variable(s) and the response variable, so that,  can estimate the value of the response Y, when only the predictors (Xs) values are known.
...
# This model has a low R-square value,  can use a transformation on the model
...
#  thus get an optimized model with R-square value of approximately 0.1231 and 
...
plot(fit_model3, 1)
# There is a pattern in the residual plot. This suggests that  cannot assume linear relationship beten the predictors and the outcome variables.
...
# In our model, not all the points fall approximately along this reference line, so  cannot assume normality.
plot(fit_model3, 2)
...
#  find that after using 10-fold cross-validation, our model accounts for 12.36% of the variance (R-squared = 0.1236544) in the data.
#  can see that the R-squared for the whole sample was approximately equal to the Cross validation results (i.e., both accounted for 12% of the variance in the shares)
# Hence  can conclude that linear regression is not a good choice for this dataset as the Adjusted R2 is very low and it does not follow the model assumptions.
